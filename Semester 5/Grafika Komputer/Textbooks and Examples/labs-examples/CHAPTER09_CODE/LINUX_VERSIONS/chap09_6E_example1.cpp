...
color4 colors[8] = {
    point4(0.0,0.0,0.0, 1.0), point4(1.0,0.0,0.0, 1.0),
    point4(1.0,1.0,0.0, 1.0), point4(0.0,1.0,0.0, 1.0),
    point4(0.0,0.0,1.0, 1.0), point4(1.0,0.0,1.0, 1.0),
    point4(0.0,1.0,1.0, 1.0), point4(1.0,1.0,1.0, 1.0)
};
...
float forces(int i, int j)
{
    ...
    if(repulsion)   /* repulsive force */
        for(k=0; k<num_particles; k++)
            if(k!=i)
                force+= 0.001*(particles[i].position[j]-
...
void myDisplay()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(points), NULL, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, buffers[1]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(point_colors), point_colors,
                 GL_DYNAMIC_DRAW);

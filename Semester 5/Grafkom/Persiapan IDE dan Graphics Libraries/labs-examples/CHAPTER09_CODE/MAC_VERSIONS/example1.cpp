/* Particles in a box */
//
// The original version of this program didn't work well -- although the user
// can increase/decrease the number of particles as well as their speed, the
// program could not regenerate the particles and animate them. The problem is
// due to the init() function being called again inside the menu callback
// function. Instead, only the buffer that contains the new data needs to be
// passed onto the vertex shader. Two new functions have been created in this
// new version:
//   (1) setup_particles() which is extracted from init() so that it can be
//       reused for generating particles using the new parameter values
//       (number, speed, gravity, etc)
//   (2) revise_particles() which takes care of setting up the particles and
//       copying the buffer to the vertex shader.
//
// Du Huynh
...
{
    point_colors[Index] = point_colors[0]; points[Index] = vertices[a]; Index++;
    point_colors[Index] = point_colors[0]; points[Index] = vertices[b]; Index++;
    point_colors[Index] = point_colors[0]; points[Index] = vertices[c]; Index++;
    point_colors[Index] = point_colors[0]; points[Index] = vertices[d]; Index++;
}
...
{
    fprintf(stderr, "#particles=%d, speed=%1.2f, ", num_particles, speed);
    fprintf(stderr, "(gravity,elastic,repulsion)=(%d,%d,%d)\n", gravity, elastic, repulsion);
}
...
void revise_particles()
{
    // redefine the particles using the new global parameter values.
    setup_particles();
...
void menu( int option )
{
    ...
    case 1:
        num_particles *= 2;
        // make sure that num_particles does not go beyond MAX_NUM_PARTICLES
        if (num_particles > MAX_NUM_PARTICLES) num_particles = MAX_NUM_PARTICLES;
        break;
    case 2:
        num_particles /= 2;
        // make sure that num_particles does not go down to 0
...
void reshape( int width, int height )
{
    ...
    point4 eye = vec4( 4.0, 4.0, -8.0, 1.0 );
    point4 at = vec4( 0.0, 0.0, 0.0, 1.0 );
    vec4 up = vec4( 0.0, 1.0, 0.0, 1.0 );

    projection = Ortho( -2.5, 2.5, 1.5, 6.5, 2.0, 16.0 );
...
int main( int argc, char **argv )
{
    ...
    // The lines below are commented to work on the Mac
